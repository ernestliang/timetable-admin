                                ...
                                <div class="row mb-3">
                                    <div class="col-12">
                                        ...
                                        <select id="location" v-model="timetable.locationId" class="form-select">
                                            ...
                                            <option v-for="loc in locationList" :key="loc.locationId" :value="loc.locationId">
                                                <span class="fw-bold">{{ loc.locationCode }}</span>-<span>{{ loc.locationDes }}</span>
                                            </option>
                                        </select>
                                    </div>
                                </div>
...
<script setup>
    ...
    function closeDialog() {
        emit('dialogClosed', null)
    }
    function saveDialog() {
        debugger
        timetable.value.subjectName = props.subjectList.find(x => x.subjectId == timetable.value.subjectId).subjectName,
        timetable.value.startTime = timetable.value.startTime.split('T')[1]
        timetable.value.endTime = timetable.value.endTime.split('T')[1]
